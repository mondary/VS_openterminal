...
```bash
"/Applications/Vibe/vibe Antigravity.app/Contents/Resources/app/bin/antigravity" --install-extension "/Users/clm/Documents/GitHub/EXTENSIONS/VS_openterminal/release/open-terminal-editor-tab-0.2.1.vsix"
```
...
- Open the **LLMs** view in the Activity Bar to toggle each button, see logos, usage badges, and open install links.
...
- The **Usage** view lists remaining usage per LLM (blank if unavailable).
- Usage data comes from the CodexBar CLI (`codexbar usage --format json`), which must be installed separately.
- Settings: `openTerminalEditor.codexbarPath` (CLI path) and `openTerminalEditor.usageRefreshMinutes` (polling interval).
...
Terminal titles include remaining usage when available.
